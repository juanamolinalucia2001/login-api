...
export default {
  ...
  data(){
    //inicializo variables para usar en el v-model
    return{
      usuario:'',
      password:'',
      error:false,
  ...
  methods:{
    login(){
      // creo json para pasarle los datos de mi usuario a la api por el post
      let json ={
        "usuario" : this.usuario,
        "password" :this.password
      };

    //usando axios le paso la api y el json con los datos de usuario
    axios.post('https://api.solodata.es/auth',json )
    .then(data =>{
      console.log(data)
      // si el estado es ok redirigo al usuario a la pantalla principal
      if(data.data.status == "ok"){
        localStorage.token = data.data.result.token;
        this.$router.push('dashboard')
      // sino muestro el error en una alerta
      }else{
        this.error=true;
